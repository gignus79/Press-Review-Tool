...
import { perplexitySearch, buildSearchQueries } from '@/lib/perplexity';
...
export async function POST(req: Request) {
  try {
    ...
    const { artist, album, language = 'en', max_results = 50, content_filter = 'All' } = body;
    ...
    const rawResults = await perplexitySearch(queries, {
    ...
    await sql`
      ...
      VALUES (${user!.id}, ${artist || null}, ${album || null}, ${language}, ${filtered.length}, ${exportId}, ${JSON.stringify({ query: { artist, album, language }, results: filtered })})
    `;
